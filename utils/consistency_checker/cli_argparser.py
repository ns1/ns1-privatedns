...
def parse_inputs():
    ...
    help_texts = {
        ...
        "control": ("IP address of the control server (known good behavior)"),
        "target": ("IP address of the target server (being tested)"),
        "multi": ("Dictate whether to use program in multi-threaded mode"),
        "file_in": ("Optionally specify a file to list all records to be tested"),
        "records": (
            "List of records to be tested: records separated by spaces and\n"
            "formated as {domain}_{type}"
        ),
        "file_out": ("File path for the results file"),
        "async": (
    ...
    server_group.add_argument(
        "-c", "--control", type=str, help=help_texts["control"], default=def_control
    )

    server_group.add_argument(
        "-t", "--target", type=str, help=help_texts["target"], required=True
    )

    mode_group.add_argument(
        "-m", "--multi_threaded", action="store_true", help=help_texts["multi"]
    )

    mode_group.add_argument(
        "-a", "--async_ops", action="store_true", help=help_texts["async"]
    )
    ...
    records_group.add_argument(
        "-r", "--records", type=str, help=help_texts["file_in"], default=def_file_in
    )

    records_group.add_argument(
        "-l", "--records_list", type=str, nargs="+", help=help_texts["records"]
    )

    results_group.add_argument(
        "-f", "--results", type=str, help=help_texts["file_out"], default=def_file_out
    )

    args = parser.parse_args()

    NUM_RECORDS = consistency_checker.get_num_records(args.records or args.records_list)
    ...
    if args.records:
        ...
        records = [(record.split("_")[0], record.split("_")[1]) for record in records]
