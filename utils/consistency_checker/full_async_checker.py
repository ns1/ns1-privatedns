...
def program_operations(records, control_server, target_server, results):
    ...
    servers = {"control": control_server, "target": target_server}
    for batch in chunker(BATCH_SIZE, records):
        loop = asyncio.get_event_loop()
        tasks = [async_operations(record, servers, results) for record in batch]
...
async def async_operations(record, servers, results):
    ...
    for server in servers:
        answers[server] = await dns_coroutine(domain, record_type, servers[server])
    ...
    try:
        ...
        if "answers" not in differences or not consistency_checker.NUM_TRIALS:
            ...
            consistency_checker.GLOBAL_QUERY_COUNT += consistency_checker.NUM_TRIALS * 2
            """
            ...
            distributions = await get_distribution(
                domain, record_type, servers, consistency_checker.NUM_TRIALS
            )
            try:
                dist_diff = consistency_checker.distribution_diff_check(distributions)
            except consistency_checker.ResponseDifference as e:
                dist_diff = e.args[0]
                consistency_checker.logging.info(
        ...
        consistency_checker.logging.debug(f"Nominal results: {domain}_{record_type}")
